...
public class StrategyAggregator {
    ...
    public static void run(JSONObject strategyData) throws IOException {
        int strategyId = 0;
        try {
            String pair = (String) strategyData.get("pair");
            String strategyType = (String) strategyData.get("strategyType");
            int primary = Integer.parseInt( String.valueOf(strategyData.get("primary")));
            int secondary = Integer.parseInt( String.valueOf(strategyData.get("secondary")));
            String emaFirstField = (String) strategyData.get("emaFirstField");
            String emaSecondField = (String) strategyData.get("emaSecondField");
            String emaOperator = (String) strategyData.get("emaOperator");
            int emaInterval = Integer.parseInt( String.valueOf(strategyData.get("emaInterval")));
            String filterType = (String) strategyData.get("filterType");
            int filterInterval = Integer.parseInt( String.valueOf(strategyData.get("filterInterval")));
            double filterRangeS = Double.parseDouble(String.valueOf(strategyData.get("filterRangeS")));
            double filterRangeE = Double.parseDouble(String.valueOf(strategyData.get("filterRangeE")));
            String bootstrapServers = (String) strategyData.get("bootstrapServers");
            strategyId = Integer.parseInt( String.valueOf(strategyData.get("strategyId")));
            String strategyName = (String) strategyData.get("strategyName");

            double tp1 = parseDoubleParam(strategyData.get("tp1"));
            double tp2 = parseDoubleParam(strategyData.get("tp2"));
            double tp3 = parseDoubleParam(strategyData.get("tp3"));
            double tp4 = parseDoubleParam(strategyData.get("tp4"));
            double tp5 = parseDoubleParam(strategyData.get("tp5"));
            double allocation = Double.parseDouble(String.valueOf(strategyData.get("filterRangeE")));
            double q1 = parseDoubleParam(strategyData.get("q1"));
            double q2 = parseDoubleParam(strategyData.get("q2"));
            double q3 = parseDoubleParam(strategyData.get("q3"));
            double q4 = parseDoubleParam(strategyData.get("q4"));
            double q5 = parseDoubleParam(strategyData.get("q5"));
...
//                    Check if the Strategy is active
                        ...
                        if (primaryBuySignalSwitch  && primaryBuySignalAgeValidator && secondaryBuySignalAgeValidator && emaFirstAgeValidator && emaSecondAgeValidator && filterAgeValidator) {
                            ...
                            if (strategyType.equals("Long")) {
                                if (Objects.equals(secondaryBuySignal, "G") && compareEmas(emaFirst, emaSecond, emaOperator)) {
                                    pushStrategy(strategyId, strategyName, producer, pair, primary, currentDatapoints.toString(),strategyData.toString());
    ...
    private static void pushStrategy(int strategyId, String strategyName, KafkaProducer producer, String pair, int primary,String currentDatapoints,String strategyData) {
        JSONObject strategyJ = new JSONObject();
        strategyJ.put("time_readable", epochToReadable(Instant.now().toEpochMilli()));
        strategyJ.put("time", Instant.now().toEpochMilli());
        strategyJ.put("strategyData", strategyData);
    ...
    private static double parseDoubleParam(Object param){
        if (param != "" && param != null) {
    ...
    private static String epochToReadable(long epoch){
        Date date = new Date(TimeUnit.MILLISECONDS.convert(epoch, TimeUnit.MILLISECONDS));
        DateFormat format = new SimpleDateFormat("d MMM yyyy HH:mm:ss.SSS");
